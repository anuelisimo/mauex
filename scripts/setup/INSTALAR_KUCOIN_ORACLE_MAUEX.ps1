$ErrorActionPreference = "Stop"

$ip = "146.235.243.105"
$hostName = "mauex-binance.146.235.243.105.nip.io"
$user = "ubuntu"
$root = Split-Path -Parent $MyInvocation.MyCommand.Path
$backendDir = Join-Path $root "oracle-binance-backend"
$backendFile = Join-Path $backendDir "mauex-binance-backend.js"
$installerFile = Join-Path $backendDir "instalar-en-oracle.sh"

Write-Host ""
Write-Host "============================================"
Write-Host "  MAUex - agregar KuCoin a Oracle"
Write-Host "============================================"
Write-Host ""
Write-Host "IP de Oracle: $ip"
Write-Host ""

if (!(Test-Path $backendFile) -or !(Test-Path $installerFile)) {
  throw "No encuentro los archivos del backend en $backendDir"
}

$defaultKey = "C:\Users\mauap\.ssh\mauex_oracle"
$keyPath = Read-Host "Private key de Oracle [Enter = $defaultKey]"
if ([string]::IsNullOrWhiteSpace($keyPath)) { $keyPath = $defaultKey }
$keyPath = $keyPath.Trim().Trim('"')
if (!(Test-Path $keyPath)) {
  throw "No encuentro la private key en: $keyPath"
}

Write-Host ""
Write-Host "Voy a ajustar permisos de la private key para que Windows permita usarla..."
icacls $keyPath /inheritance:r | Out-Null
icacls $keyPath /grant:r "$($env:USERNAME):R" | Out-Null

Write-Host ""
$kucoinKey = Read-Host "Pega KUCOIN_KEY"
$secureSecret = Read-Host "Pega KUCOIN_SECRET" -AsSecureString
$securePassphrase = Read-Host "Pega KUCOIN_PASSPHRASE" -AsSecureString

$secretPtr = [Runtime.InteropServices.Marshal]::SecureStringToBSTR($secureSecret)
$passPtr = [Runtime.InteropServices.Marshal]::SecureStringToBSTR($securePassphrase)
try {
  $kucoinSecret = [Runtime.InteropServices.Marshal]::PtrToStringBSTR($secretPtr)
  $kucoinPassphrase = [Runtime.InteropServices.Marshal]::PtrToStringBSTR($passPtr)
} finally {
  [Runtime.InteropServices.Marshal]::ZeroFreeBSTR($secretPtr)
  [Runtime.InteropServices.Marshal]::ZeroFreeBSTR($passPtr)
}

$tempId = [Guid]::NewGuid().ToString("N")
$tempEnv = Join-Path $env:TEMP "mauex-kucoin-$tempId.env"
$tempRemoteScript = Join-Path $env:TEMP "mauex-install-kucoin-$tempId.sh"

$envText = "KUCOIN_KEY=$kucoinKey`nKUCOIN_SECRET=$kucoinSecret`nKUCOIN_PASSPHRASE=$kucoinPassphrase`nKUCOIN_KEY_VERSION=2`n"
$remoteScript = @'
set -e
cd /tmp/mauex-oracle
sudo bash instalar-en-oracle.sh
sudo touch /opt/mauex-binance/.env
sudo sh -c "grep -v '^KUCOIN_' /opt/mauex-binance/.env > /tmp/mauex-env && cat /tmp/mauex-oracle/kucoin.env >> /tmp/mauex-env && cp /tmp/mauex-env /opt/mauex-binance/.env && chmod 600 /opt/mauex-binance/.env"
sudo systemctl restart mauex-binance
sudo systemctl status mauex-binance --no-pager -l | head -n 20
'@

try {
  Set-Content -Path $tempEnv -Value $envText -Encoding ASCII -NoNewline
  Set-Content -Path $tempRemoteScript -Value $remoteScript -Encoding ASCII -NoNewline

  Write-Host ""
  Write-Host "Subiendo backend actualizado a Oracle..."
  ssh -i $keyPath -o StrictHostKeyChecking=accept-new "$user@$ip" "mkdir -p /tmp/mauex-oracle"
  scp -i $keyPath -o StrictHostKeyChecking=accept-new $backendFile $installerFile "${user}@${ip}:/tmp/mauex-oracle/"
  scp -i $keyPath -o StrictHostKeyChecking=accept-new $tempEnv "${user}@${ip}:/tmp/mauex-oracle/kucoin.env"
  scp -i $keyPath -o StrictHostKeyChecking=accept-new $tempRemoteScript "${user}@${ip}:/tmp/mauex-oracle/instalar-kucoin.sh"

  Write-Host ""
  Write-Host "Instalando KuCoin en Oracle sin tocar las claves de Binance..."
  ssh -i $keyPath -o StrictHostKeyChecking=accept-new "$user@$ip" "bash /tmp/mauex-oracle/instalar-kucoin.sh"

  Write-Host ""
  Write-Host "Probando servicio..."
  Start-Sleep -Seconds 3

  $health = Invoke-RestMethod "http://${hostName}:8080/health" -TimeoutSec 20
  Write-Host "Health OK:" ($health | ConvertTo-Json -Compress)

  $balance = Invoke-RestMethod "http://${hostName}:8080/kucoin-balance" -TimeoutSec 25
  Write-Host "Respuesta KuCoin:" ($balance | ConvertTo-Json -Compress)

  Write-Host ""
  Write-Host "Siguiente paso:"
  Write-Host "En Cloudflare Worker agrega KUCOIN_BACKEND_URL=http://${hostName}:8080"
  Write-Host "Si ya tenes BINANCE_BACKEND_URL con ese mismo valor, tambien sirve."
} finally {
  Remove-Item -LiteralPath $tempEnv -Force -ErrorAction SilentlyContinue
  Remove-Item -LiteralPath $tempRemoteScript -Force -ErrorAction SilentlyContinue
}

Write-Host ""
Read-Host "Presiona Enter para cerrar"


