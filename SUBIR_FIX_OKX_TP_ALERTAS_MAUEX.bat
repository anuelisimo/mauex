...
chcp 65001 >nul
setlocal
title MAUex - fix OKX y alertas TP
cd /d "%~dp0"

echo.
echo ============================================
echo   MAUex - fix OKX y alertas TP
echo ============================================
echo.
echo Este BAT sube:
echo   worker.js
...
echo Al final copia worker.js al portapapeles para deploy manual si hace falta.
echo.
...
set "COMMIT_MSG=%~1"
if "%COMMIT_MSG%"=="" set "COMMIT_MSG=Fix OKX diagnostic and TP alerts"
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$v=Get-Date -Format 'yyyyMMdd-HHmmss'; $p='index.html'; $s=Get-Content -LiteralPath $p -Raw; $s=[regex]::Replace($s,'firebase\.js\?v=[0-9A-Za-z._-]+','firebase.js?v='+$v); Set-Content -LiteralPath $p -Value $s -NoNewline -Encoding UTF8; Write-Host ('Version aplicada: '+$v)"
if errorlevel 1 (
  echo.
...
git add -- worker.js firebase.js index.html SUBIR_FIX_OKX_TP_ALERTAS_MAUEX.bat
if errorlevel 1 (
  echo.
...
echo.
echo Si necesitas aplicar Worker:
