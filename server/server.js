...
try {
  ...
  if (!admin.apps.length) {
    if (!process.env.FIREBASE_SERVICE_ACCOUNT) {
      throw new Error('FIREBASE_SERVICE_ACCOUNT env var is required for Firestore access');
    }
...
const allowedOrigin = (process.env.ALLOWED_ORIGIN || DEFAULT_ALLOWED_ORIGIN).trim();
...
app.use(cors({
  origin(origin, callback) {
    callback(isAllowedOrigin(origin) ? null : new Error('CORS origin denied'), isAllowedOrigin(origin));
  },
...
function requireApiToken(req, res, next) {
  ...
  if (!expected || !timingSafeEqualText(got, expected)) {
