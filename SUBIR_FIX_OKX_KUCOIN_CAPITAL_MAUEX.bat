...
chcp 65001 >nul
setlocal
title MAUex - fix OKX Kucoin capital
cd /d "%~dp0"

echo.
echo ============================================
echo   MAUex - fix OKX / Kucoin capital
echo ============================================
echo.
echo Este BAT sube:
echo   app.js
...
set "COMMIT_MSG=%~1"
if "%COMMIT_MSG%"=="" set "COMMIT_MSG=Fix OKX y Kucoin en capital exchanges"
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$v=Get-Date -Format 'yyyyMMdd-HHmmss'; $p='index.html'; $s=Get-Content -LiteralPath $p -Raw; $s=[regex]::Replace($s,'app\.js\?v=[0-9A-Za-z._-]+','app.js?v='+$v); Set-Content -LiteralPath $p -Value $s -NoNewline -Encoding UTF8; Write-Host ('Version aplicada: '+$v)"
if errorlevel 1 (
  echo.
...
git add -- app.js index.html worker.js SUBIR_FIX_OKX_KUCOIN_CAPITAL_MAUEX.bat
if errorlevel 1 (
  echo.
